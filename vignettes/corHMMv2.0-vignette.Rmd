...
homo_gorilla <- getMRCA(tree,tip=c("Homo_sapiens", "Gorilla_gorilla"))
homo_gorilla
```
...
```{r, echo=FALSE}
fix.node.test <- corHMM(tree, trait[,c(1,2)], model="ER", rate.cat=1, fixed.nodes=TRUE)
```


with a user-provided tree that has states coded at nodes under a maximum parsimony reconstruction. However, with **phangorn**, there is the burden of dealing with their unique `phyDat` format. To deal with this, we implemented a function that will convert the `phyDat` formatted output into something we can modify and input into `corHMM`. For example, we can take the `mpr.recon` object from **phangorn** and convert the output as a vector and add them as node states in the phylogeny: 
...
library(phangorn)
data.sort <- data.frame(trait[,2], row.names=trait[,1])
...
mpr.recon <- ancestral.pars(tree, dat, type = c("MPR"))
mpr.recon.converted <- ConvertPhangornReconstructions(mpr.recon)
tree$node.label <- mpr.recon.converted[(Ntip(tree)+1):length(mpr.recon.converted)]
```
...
Note that node labels are not the same as the tip states. This is because they are meant to be indicators for the order of the states. As you see in the tree above the 1's and 2's at nodes, where a 1 is an indicator for state 0 and a 2 is an indicator for state 1. At first glance this might seem silly, but the reason it is implemented this way is so that the input data does not necessarily have to be a vector of numerics, and could be names, etc. -- though life is a lot easier if it is in the same format as the primates example. In any event, in general, the indicators at the nodes point to the order of the input names as if they were factors:

```{r, eval=FALSE}
names.vector <- c("beta", "alpha", "alpha", "beta", "beta")
names.factor <- as.factor(names.vector)
as.numeric(names.factor)
```

Now that we have our reconstructions, we can obtain the likelihood of this reconstruction *given* a set of rates. To do this we rely on the joint reconstruction algorithm of Pupko et al (2000), which represents the maximum likelihood estimates of the states on nodes. To obtain the reconstruction likelihood we've added a new argument to the rayDISC function called `state.recon`. The default is `state.recon='subsequently'`, which performs the usual `rayDISC` call -- that is, it first estimates the rates, then uses those rate estimates to obtain the likeliest states. However, if you set `state.recon= 'same'`, it will provide the likelihood that is based on the input reconstruction and the rates. For example, for a fixed set of rates we can obtain the likelihood of our parsimony reconstruction:

```{r, eval=TRUE}
recon.likelihood <- rayDISC(tree, trait,model="ER", p=c(0.008), 
                            node.states="joint", state.recon="given", root.p=NULL)
recon.likelihood
```

Here the likelihood is based on a fixed set of rates being 0.008, which is provided in the `p` argument. If we wanted to actually *estimate* the likeliest rate given the parsimony reconstruction, all we have to do is omit the `p` argument entirely:

```{r, eval=TRUE}
recon.rate.best.likelihood <- rayDISC(tree, trait,model="ER", 
                                      node.states="joint", state.recon="given", root.p=NULL)
recon.rate.best.likelihood
```

As shown in this example, the estimated rates are nearly order of magnitude greater than our input value. 

If a user wanted to estimate both the rates and states all in one function call, simply provide a tree that has no internal node labels:

```{r, eval=TRUE}
tree.no.labels <- tree
tree.no.labels$node.label <- NULL
recon.rate.state.best.likelihood <- rayDISC(tree.no.labels, trait,model="ER", 
                                            node.states="joint", state.recon="estimate", root.p=NULL)
recon.rate.state.best.likelihood
```

Here is a plot of the likeliest states at nodes:

```{r, echo=FALSE}
plot(recon.rate.state.best.likelihood$phy)
nodelabels(recon.rate.state.best.likelihood$phy$node.label)
```

We also allow for partial knowledge of states to be included. Perhaps we want to estimate both rates and states, but with a particular node state fixed. For example, say we wanted to determine the likeliest rates and states with the MRCA of *Homo sapiens* and *Pan paniscus* to be in state 1 :

```{r, eval=TRUE}
tree.no.labels <- tree
label.vector <- rep(NA, Ntip(tree) + Nnode(tree))
#get MRCA of H. sapiens and P. paniscus:
homo_pan <- getMRCA(tree,tip=c("Homo_sapiens", "Pan_paniscus"))
label.vector[homo_pan] <- 2
tree.no.labels$node.label <- label.vector[(Ntip(tree.no.labels)+1):(Ntip(tree)+Nnode(tree))]
recon.rate.state.best.likelihood <- corHMM(tree.no.labels, trait[,c(1,2)], model="ER", rate.cat=1)
...
## Q2: How biased a character’s evolution has to be in order to make a less parsimonious reconstruction more likely than a parsimonious one. 

Incidentally, **corHMM** already has this capability. As part of the function call is the `p` argument that supplies fixed rates. Going back to our primate example above we can show how we can modify rates and examine how this impacts the reconstructions.

```{r, eval=TRUE}
library(corHMM)
data(primates)
tree <- primates$tree
trait <- primates$trait
MLE.est <- rayDISC(tree.no.labels, trait,model="ER", p=c(0.008522092), 
                   node.states="joint", root.p=NULL)
```

The likeliest states at each node under the MLE of the equal rates model is as follows:

```{r, echo=FALSE}
plot(tree)
nodelabels(MLE.est$states)
```

Now let's see what the reconstruction looks like when the rate is 1000x that of the MLE

```{r, eval=TRUE}
crazy.train <- rayDISC(tree.no.labels, trait,model="ER", p=c(0.008522092)*1000, 
                       node.states="joint", root.p=NULL)
```

```{r, echo=FALSE}
plot(tree)
nodelabels(crazy.train$states)
```

Under this scenario, the reconstructions are much more complicated than what is inferred under the MLE.
