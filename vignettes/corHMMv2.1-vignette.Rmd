---
title: "corHMM 2.1: Completely generalized hidden Markov models"
...
* __Introduction Some background information__
...
The final new plotting tool we have available to users is a substitution history plot. This is based on the theory of Bollback (2006). We can use `makeSimmap` to create a substituion history for any corHMM model and then use plotSimmap (from the popular R-package, phytools) to plot the output. 
...
And again we can plot the simmap of this corHMM result. It is important to note that a substitution history not only generates hypotheses about ancestral states, but is an effective way to visualize the tempo of evolution. This is particularly important for HMMs where rates of evolution can vary drastically across the tree.
