...
In a recent publication (O'Meara and Beaulieu 2024), we found that simple character misassignments create upward biases in rates, which becomes increasingly dramatic in short time intervals or as clade ages approach the present. We initially termed these misassignments as "errors". However, there are a variety of processes that could lead to a misassigned character state. For example, some species may be observed as predominantly in one state but is also observed in the alternative state on occasion. In order to remain neutral with regards to the cause of the misassignments we coined the phrase "tip fog" (see Beaulieu and O'Meara, Submitted). 

To demonstrate the impact of tip fog on estimated transition rates, we will simulate a simple two state character on a balanced tree containing 64 taxa:
```{r}
set.seed(1980)
...
and then estimate the transition rates using `corHMM`:
```{r}
pp.nofog <- corHMM(phy, phydat, model="ER", rate.cat=1)
pp.nofog
```

Here when the rates are known perfectly we obtain a rate estimate of ~0.025 transitions per million years. Now let's see what happens when we choose 10% of the taxa and switch their observed state:
```{r}
error <- 0.10
...
To see the effect of tip fog let's re-estimate the transition rates:
```{r}
pp.fog <- corHMM(phy, phydat.new, model="ER", rate.cat=1)
pp.fog
```

The transition rate increases by an order of magnitude. We can run a fix value for tip fog of 10%. Let's run it:
```{r}
pp.fixfog <- corHMM(phy, phydat.new, model="ER", rate.cat=1, tip.fog=0.1)
pp.fixfog
```

We can also estimate the fog. To do this, the user must input a vector of the same length as the observed states in the data set that contains the number of free parameters the user wants to estaimte. For example, suppose we want to assume a single tip fog for both states, we'd input a vector of `c(1,1)` to correspond to a single parameter applied to both states:
```{r}
pp.estfog1 <- corHMM(phy, phydat.new, model="ER", rate.cat=1, tip.fog=c(1,1))
pp.estfog1
```

Here the estimated fog value is 0.091, meaning that a given tip has about 9% chance of being incorrectly assigned it's observed state. This is very close to the 10% we used in the simulation. Now, suppose we want to see if there are different tip fog values for each of the observed states, we'd input a vector of `c(1,2)`:
```{r, echo=FALSE}
pp.estfog2 <- corHMM(phy, phydat.new, model="ER", rate.cat=1, tip.fog=c(1,2), fog.ip=0.09095415)
```
...
Under this model we see that state 1 has a slightly lower fog probability than state 2. However, looking at the AIC between the single fog versus and multiple fog estimates, the additional complexity does not improve the fit. 

One last thing to note: for models that include hidden states, the tip fog values for each observed state are simply replicated across the different rate categories. Thus, a user only needs to include the vector for the free parameters for the observed state. For example, in the two tip fog model, but with two rate categories the input would be:

```{r}
pp.estfog2.2cat <- corHMM(phy, phydat.new, model="ER", rate.cat=2, tip.fog=c(1,2))
pp.estfog2.2cat
```
...
As of 5/19 this functionality is still being tested thoroughly and subject to change. 
