...
In a recent publication (O'Meara and Beaulieu 2024), we found that simple character misassignments create upward biases in rates, which becomes increasingly dramatic in short temporal intervals or as clade ages approach the present. We initially termed these misassignments as "errors". However, there are a variety of processes that could lead to a misassigned character. For example, some species may be observed as predominantly in a particular state but is also observed in the alternative state on occasion. In order to remain neutral with regards to the cause of the misassignments we coined the phrase "tip fog". 

To demonstate the impact of tip fog on estimated transition rates, we will simulate a simple two state character on a balanced tree containing 64 taxa:
```{r}
set.seed(1980)
...
```{r}
pp.nofog <- corHMM(phy, phydat, model="ER", rate.cat=1, tip.fog=0)
pp.nofog
```
...
To see the effect of tip fog let's reestimate the transition rates:
```{r}
pp.fog <- corHMM(phy, phydat.new, model="ER", rate.cat=1, tip.fog=0)
pp.fog
```

The transition rate increases by an order of magnitude. The new default is to assume a tip fog of 10%. Let's run the new default:
```{r}
pp.fixfog <- corHMM(phy, phydat.new, model="ER", rate.cat=1)
pp.fixfog
```
...
Here the estimated fog value is 0.091, meaning that a given tip has about 9% change of being incorrectly assigned it's observed state. This is very close to the 10% we used in the simulation. Now, suppose we want to see if there are different tip fog values for each of hte observed sates, we'd input a vector of `c(1,2)`:

```{r}
pp.estfog2 <- corHMM(phy, phydat.new, model="ER", rate.cat=1, tip.fog=c(1,2))
pp.estfog2
```
